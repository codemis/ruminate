...
.controller('PassageController', ['$scope', 'ParseReflection', function($scope, ParseReflection){
  
  // $scope.passage = ;

}])
.controller('HomeController', [ '$scope', '$ionicModal', '$location', 'ParseReflection', 'ParsePassage', 'ParseResponse', 'ParseQuestion'
  ,function($scope, $ionicModal, $location, ParseReflection, ParsePassage, ParseResponse, ParseQuestion) {
  
  ParseReflection.getForToday(function(reflections) {
    $scope.reflection = reflections[0];
    console.log($scope.reflection);
    ParsePassage.getFromReflection($scope.reflection, function(passage) {
      console.log(passage);
      $scope.passage = passage;
      $scope.truncatedSnippet = truncate(passage.snippet, 320);
    });
